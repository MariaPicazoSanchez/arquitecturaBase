...
// Configurar Express: servir archivos est+�ticos desde /client
app.use(express.static(path.join(__dirname, 'client')));

// P+�gina de reset password (link desde email)
...
// CORS (solo para /api) cuando el cliente est+� en otro origen y APP_URL est+� configurada.
...
app.get("/good", function(req, res) {
  ...
  process.nextTick(() => {
    sistema.usuarioGoogle({ email, displayName }, function(obj) {
      ...
      if (!obj || !obj.email) {
        logger.error("[/good] ERROR: objeto inv+�lido de usuarioGoogle");
        return res.redirect('/fallo');
      }
      try {
...
app.get('/salir', function(req, res){
  ...
  try{
    // Passport: intenta logout si est+� disponible
...
// Diagnostic endpoint: listar archivos est+�ticos desplegados (+�til en producci+�n)
